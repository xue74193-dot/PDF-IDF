...
public:
    Edge(){};
    Edge(int a,int b,double w,double c)
    {
        x=a;
        y=b;
        weight=w;
...
bool is_planar(const int &n,const vector<Edge> &edge_set) {
    int m=edge_set.size();
    vector<Graph> Div;
    ...
    {
//        if (e.x == e.y) continue;
//        Tot.add(e.x, e.y), Tot.add(e.y, e.x);
        if (e.x == e.y) continue;
        Tot.add(renum[e.x], renum[e.y]), Tot.add(renum[e.y], renum[e.x]);

        cout<<"original edge: "<<"("<<e.x<<","<<e.y<<")"<<" "<<endl;
        cout<<"new edge: "<<"("<<renum[e.x]<<","<<renum[e.y]<<")"<<" "<<endl;
    }
    //n is only used there
    if (m > 3 * n - 6 && m > 1) {
        return false;
    }
    ...
    for (int i = 0; i < Div.size(); i++) {
        ...
        for (int j = 1; j <= sta[0]; j++)
            for (int l = Div[i].head[sta[j]]; l; l = Div[i].next[l])
                if (Div[i].val[l]) {
                    Graph temp;
