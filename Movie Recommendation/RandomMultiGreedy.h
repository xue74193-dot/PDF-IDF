...
//}
pair<int,double> GetMax(long long int &oracle_times,double eps,multimap<double,Ai_node> &A,S_gamma &Si,const vector<int> &available,const int &max_node)
{
...
pair<S_gamma,long long int> multi_random_acc(double eps,dataset* data,const vector<int> &N,const int &max_node)
{
    ...
    while(1)
    {
        ...
        if(u(e)==1)
        {
            S[max_solution].add_element(max_marginal,max_element,data);
        }
...
Result RandomMultiGreedy(double eps,dataset* data,const vector<int> &N,const int &max_node)
{
...
//    for(int j=0;j<(*S_star).genres_sum.size();j++)
//    {
//        cout<<(*S_star).genres_sum[j]<<" ";
//    }
//    cout<<endl;
