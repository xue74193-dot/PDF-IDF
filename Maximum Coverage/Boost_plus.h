...
//}
pair<int,double> GetMax_for_boost(long long int &oracle_times,double eps,multimap<double,Ai_for_boost> &A,S_class &Si,const vector<int> &available,const int &max_node,const double &ratio)
{
/********check this when pop a node*************/
    ...
    while(!A.empty())
    {
        ...
        oracle_times++;
        double new_value=Si.marginal_gain_distorted(it->second.node,ratio);
...
pair<S_class,long long int> Boost(double eps,const vector<int> &N,const int &max_node,const double &ratio,pair<int,double> u_star)
{
    ...
    //S<-S\cup {u^*}
    S[0].add_element(u_star.second,u_star.first);
    ...
    while(1)
    {
        for (int j = 0; j < S.size(); j++)
        {
            ...
            pair<int,double> temp=GetMax_for_boost(oracle_times,eps,A[j],S[j],available,max_node,ratio);
        ...
        if (max_solution == -1||max_marginal<=0)//non element or marginal gain<=0
            break;
        //S<-S\cup {u}
        double real_marginal=max_marginal+(ratio-1.0)*Groundset[max_element].cost;
        S[max_solution].add_element(real_marginal,max_element);
